...
from services.ai_mapper import AIMapperError, GeminiAIMapper
...
def _fallback_sql(question: str, session_meta: Dict[str, Any]) -> str:
    lower_q = question.lower()
    columns = session_meta.get("columns", [])

    if "how many" in lower_q or "count" in lower_q or "total" in lower_q:
        return "SELECT COUNT(*) AS total_records FROM dataset"

    if "distinct" in lower_q or "unique" in lower_q:
        if columns:
            return f"SELECT COUNT(DISTINCT \"{columns[0]}\") AS unique_values FROM dataset"
        return "SELECT COUNT(*) AS total_records FROM dataset"

    if "top" in lower_q or "most" in lower_q:
...
def _fallback_answer(question: str, sql: str, result_rows: List[Dict[str, Any]]) -> str:
    ...
    return f"Found {len(result_rows)} matching records in the dataset."
...
async def clean_upload(
    ...
    try:
        ...
        background_tasks.add_task(_run_clean_job, job_id, filename, raw)
...
def query_data(payload: QueryRequest) -> dict:
    ...
    try:
        ...
        try:
            ...
            sql = _fallback_sql(question, session_meta)
