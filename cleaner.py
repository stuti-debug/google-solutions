...
class DataCleaner:
    def __init__(self, mapper: GeminiAIMapper, chunk_size: int = 50, ai_validation_retries: int = 2):
        ...
        self.ai_validation_retries = max(0, ai_validation_retries)
    ...
    def _clean_with_ai_chunks(
        ...
        total_chunks = math.ceil(len(records) / self.chunk_size)
        ...
        for i in range(total_chunks):
            ...
            chunk_result = self._clean_chunk_with_ai(
                chunk_rows=chunk,
                file_type=file_type,
            ...
            cleaned_rows.extend(chunk_result.cleanedRows)
            ...
            chunk_progress = 30 + int(((i + 1) / max(total_chunks, 1)) * 50)
            self._emit_progress(progress_callback, chunk_progress, f"Processed chunk {i + 1}/{total_chunks}")
